...
import { createColor, getContrastRatio, meetsAAContrast, adjustForContrast, checkColorBlindnessDistinction } from './colorUtils.js';
const ICON_STYLES = [
    'outline',
    'rounded',
    'sharp',
...
export class PaletteEngine {
    ...
    generateSinglePalette(input, index) {
    ...
    adjustForTheme(colors, theme) {
        const backgroundLightness = theme === 'light' ? 0.95 : theme === 'dark' ? 0.1 : 0.5;
        // Adjust background
        ...
        // Ensure contrast for other colors against new background
        ...
        for (const role of ['primary', 'secondary', 'accent', 'neutral']) {
            ...
            if (!meetsAAContrast(contrast)) {
                ...
                const adjustedColor = adjustForContrast(chroma(color.hex), bgLuminance, 4.5, targetLightness > color.hsl[2] / 100);
                colors[role] = createColor(adjustedColor.hex(), role);
            }
        }
    }
    ensureAccessibility(colors) {
        ...
        for (const role of ['primary', 'secondary', 'accent', 'neutral']) {
            const color = colors[role];
            let contrast = getContrastRatio(color.luminance, bgLuminance);
            let attempts = 0;
            while (!meetsAAContrast(contrast) && attempts < 10) {
                // Adjust lightness
                const currentLightness = color.hsl[2] / 100;
                const adjustment = bgLuminance > 0.5 ? -0.1 : 0.1;
                const newLightness = Math.max(0.1, Math.min(0.9, currentLightness + adjustment));
                colors[role] = this.generateColor(color.hsl[0], color.hsl[1] / 100, newLightness, role);
                contrast = getContrastRatio(colors[role].luminance, bgLuminance);
                attempts++;
            }
        }
