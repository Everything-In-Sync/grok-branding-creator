...
export function meetsAAContrast(ratio: number, isLargeText: boolean = false): boolean {
  return isLargeText ? ratio >= 3.0 : ratio >= 4.5
}

// Check if contrast meets WCAG AAA requirements
export function meetsAAAContrast(ratio: number, isLargeText: boolean = false): boolean {
...
export function getBestTextColor(backgroundLum: number): TextColor {
  return backgroundLum > 0.5 ? 'dark' : 'light'
}

// Create a Color object from hex
export function createColor(hex: string, role: string): Color {
...
export function checkColorBlindnessDistinction(color1: string, color2: string): boolean {
  const types: ('protanopia' | 'deuteranopia' | 'tritanopia')[] = ['protanopia', 'deuteranopia', 'tritanopia']
