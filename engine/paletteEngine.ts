...
import { GenerateInput, Palette, Color, BrandTone, ThemePreference } from '../shared/types.js'
...
import {
  ...
  meetsAAContrast,
  adjustForContrast,
  checkColorBlindnessDistinction
} from './colorUtils.js'
...
export class PaletteEngine {
  ...
  private generateSinglePalette(input: GenerateInput, index: number, context?: any): Palette {
  ...
  private selectHarmonyType(tone?: BrandTone): 'analogous' | 'complementary' | 'triadic' | 'split-complementary' {
    const harmonyOptions: ('analogous' | 'complementary' | 'triadic' | 'split-complementary')[] = ['analogous', 'complementary', 'triadic', 'split-complementary']
  ...
  private generateColor(hue: number, saturation: number, lightness: number, role: string): Color {
  ...
  private adjustForTheme(colors: any, theme: ThemePreference): void {
    if (theme === 'neutral') {
  ...
  private ensureAccessibility(colors: any): void {
    const bgLuminance = colors.background.luminance
  ...
  private generatePaletteName(industry: string, tone: BrandTone | undefined, index: number): string {
  ...
  private generateImageryDescription(): string[] {
  ...
  private generateLogoPrompts(industry: string, tone?: BrandTone): string[] {
    const prompts = [
